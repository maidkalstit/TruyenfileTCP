 ...
 * Server này sử dụng mô hình Singleton và chạy trên port có thể tùy chỉnh.
 */
public class Server {
    ...
    /**
     * Đặt port cho server (phải gọi trước start()).
     * @param port Port cần đặt
     */
    public synchronized void setPort(int port) {
        if (running) {
            log("Cannot change port while server is running");
            return;
        }
    ...
    /**
     * Khởi động server trên port đã được đặt.
